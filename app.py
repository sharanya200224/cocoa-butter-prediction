...
def determine_skin_type(quality_score, oleic_acid, viscosity):
    ...
    elif oleic_acid > 33.0 and viscosity > 300 and quality_score >= 50:
        return 'oily', 'High oleic acid and viscosity may suit oily skin with proper refining.'
    elif (viscosity < 250 or quality_score < 50) and not (oleic_acid > 33.0 and viscosity > 300):
...
def generate_recommended_products(skin_type, quality_category):
    ...
    return products_by_type.get(skin_type, products[quality_category])
...
def analyze():
    try:
        data = request.json
        print(f"Received data: {data}")  # Log all input data
        ...
        print(f"Predicted quality_score: {quality_score}")  # Log the scaled quality score
        ...
        print(f"Detected skin_type: {skin_type}, quality_score: {quality_score}, oleic_acid: {data['oleic_acid']}, viscosity: {data['viscosity']}")  # Log decision factors
